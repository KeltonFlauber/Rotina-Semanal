...
class TaskLWeekAdapter(
    ...
    fun setCurrentData(query: String, query2: String,
                       query3: String, query4: String, query5: String){

        taskList.clear()
        taskList.addAll(taskListEntry.filter { it.dayOfWeek.contains(query) })
        taskList.addAll(taskListEntry.filter { it.dayOfWeek.contains(query2) })
        taskList.addAll(taskListEntry.filter { it.dayOfWeek.contains(query3) })
        taskList.addAll(taskListEntry.filter { it.dayOfWeek.contains(query4) })
        taskList.addAll(taskListEntry.filter { it.dayOfWeek.contains(query5) })
